...
// api key generated and integrated
const request = "?api_key=XxDS1iklZl7VMxHQ6ZfbWnlYoXgFOFvQFfCm2Doa&page=1";

const roverData = new Map();

// asynchronous getJson() function that makes API requests goes here
const getJson = async url => {
    console.log("Request URL:", url); // Step 5: Log the request URL
...
$(document).ready(async () => {
    ...
    $("#rover").html(roverOptions);

    // change event handler for rover dropdown
    $(document).on('change', "#rover", evt => {
        clearPrevious();
        ...
        if (data) {
            ...
            $("#year").html(getOptionHtml(landingDateParts[0], maxDateParts[0], maxDateParts[0]));
            $("#month").html(getOptionHtml(1, 12, maxDateParts[1]));
            $("#date").html(getOptionHtml(1, 31, maxDateParts[2]));
